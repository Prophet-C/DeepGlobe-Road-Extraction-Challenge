...
class GlobalLocalConv2Norm(nn.Module):
    def __init__(self, dim, h=14, w=8, kernel_size=3):
        super().__init__()
...
class GNConvModule(nn.Module):
    def __init__(self, 
        ...
        if type == 'gnconv':
            ...
            self.dwconv = GlobalLocalConv2Norm(sum(self.dims), h=h, w=w, kernel_size=gf_kernel_size) # nn.Conv2d(sum(self.dims), sum(self.dims), kernel_size=7, padding=3, groups=sum(self.dims))
        else:
            raise NotImplementedError()
